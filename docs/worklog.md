...
file is located in the `plugin` directory (generated by using `Build` > `Prepare
All Plugin Modules For Deployment`)
- I've collected four sample XML files for the sample Java application (a bingo
caller). These files can be found in the `samples` directory. Two of the samples
did not correctly track the beginning of the project. The `public class ... { }`
declaration was not recorded. Using these XML files I was able to write a small
algorithm to reconstruct each of the files using the list of changes. This works
